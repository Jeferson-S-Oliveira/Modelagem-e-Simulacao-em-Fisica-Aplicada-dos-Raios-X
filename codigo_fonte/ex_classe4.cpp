...
public:
    ...
    XYZ operator +(XYZ v) { // soma vetorial "operator +" redefine o sinal de soma para dois objetos XYZ -> o objeto da direita (XYZ) é passado como argumento para o operador + 
        return XYZ( x+v.x, y+v.y, z+v.z );
    }
    XYZ operator *(XYZ v) { // produto escalar
        return x*v.x + y*v.y + z*v.z;
    }
    XYZ operator *(double d) { // produto escalar
