...
public:
   ...
   void Rotacao(double ang, XYZ nn) {
      for ( int i=0; i<nv; i++ ) {
         v[i] = v[i].Rotacao(ang, nn);
      }
      n = n.Rotacao(ang,nn);
      d = -n*v[0];
   }
...
public:
   ...
   void Rotacao(double ang, XYZ n) {
      for (int i=0; i<nf; i++) f[i].Rotacao(ang,n);
   }

   void Imprimir(void) {
      if (f) for (int i=0; i<nf; i++ ) f[i].Imprimir();
   }
};
...
class matriz {
private:
   int    *ctg;    // matriz de contagem
   int     nx, ny; // numero de linhas e colunas
   double  lx, ly; // dimensoes físicas da matriz
   XYZ     u, v;   // vetores de suporte
   XYZ    *vf;     // lista de vertices da face
   face    f;      // face

public:
   matriz(void) {}
   matriz(double iw, double ih, int ix, int iy, XYZ ip, XYZ id) { 
      XYZ p, d;

      lx = iw; ly = ih;
      nx = ix; ny = iy;
      ctg = new int[nx*ny];
      memset(ctg,0,nx*ny*sizeof(int));

      // posicao e direcao
      p = ip; d = id;
      // eixo horizontal
      u = (XYZ(0,0,1)^d).Unitario();
      // eixo vertical
      v = u^d;

      vf = new XYZ[4];
      vf[0] = p - 0.5*lx*u - 0.5*ly*v;
      vf[1] = p - 0.5*lx*u + 0.5*ly*v;
      vf[2] = p + 0.5*lx*u + 0.5*ly*v;
      vf[3] = p + 0.5*lx*u - 0.5*ly*v;
      f = face(4,vf);
   }

   void Registrar(XYZ r, XYZ s) {
      int i, j;
      InfoGeo pt = f.Intersecao(r,s);

//      printf("u: ");u.Imprimir();
//      printf("v: ");v.Imprimir();
//      printf("matriz: pt.ok  %d\tpt.q= ",pt.ok); pt.q.Imprimir();
      if (pt.ok) {
         i = int(((pt.q-vf[0])*v)*(ny/ly));
         j = int(((pt.q-vf[0])*u)*(nx/lx));
//         printf("i: %d\tj: %d\t",i,j);
...
//            printf("\n");
   ...
   void Salvar(const char *nome_img) {
      FILE *fp;
      int i, j, val=0;

      for ( i=0; i<nx*ny; i++ ) {
         if (val<ctg[i]) val = ctg[i];
      }
      fp = fopen(nome_img,"wt");
      fprintf(fp,"P2 %d %d %d\n",nx, ny, val);
      for ( i=0; i<ny; i++ ) {
         for ( j=0; j<nx; j++ ) {
            fprintf(fp,"%8d",ctg[i*nx+j]);
         }
         fprintf(fp,"\n");
      }
      fclose(fp);
...
public:
   ...
   foton Emitir(void) {
      ...
      eps = rand()/(double)RAND_MAX;
      r = d.Rotacao( atan(sqrt(tan(theta)*eps)), d^XYZ(0,0,1) );
   ...
   void Imprimir(void) {
      ...
      printf("Abertura: %lf rad (%lf graus)\n", theta, theta/graus);
      printf("=====================================================\n");
...
public:
   ...
   void AbsFotoeletrica(foton *hv) {
      hv->Hv(0); // zera a energia; absorvido
   }
   ...
   void EspalhamentoElastico(foton *hv) {
      ...
      XYZ d, r, s;
      // SCD Thomson
      // dsigma/dtheta = re^2*(1+cos^2(theta))/2
      // P(theta) = 0.5*(1+cos^2(theta))
      do {
   ...
   void EspalhamentoInelastico(foton *hv) {
      ...
      // SCD Compton
      // k = E/(me*c^2)=E/511;
      // Ec = E/(1+k*(1-cos(theta)))
      // dsigma/dtheta = re^2*(Ec/E)(Ec/E + E/Ec - sin^2(theta))/2
      // P(theta) = 0.5*(Ec/E)*(Ec/E + E/Ec - sin^2(theta))
      k = hv->Hv()/511;
      do {
         theta = 2*M_PI*rand()/(double)RAND_MAX;
         Ec_E = 1/(1+k*(1-cos(theta)));
         Pcp = 0.5*(Ec_E)*(Ec_E)*(Ec_E + 1/Ec_E - sin(theta)*sin(theta));
         eps = rand()/(double)RAND_MAX;
      } while (eps>Pcp);
...
//   db.Imprimir();
   ...
   sol.Rotacao(45*graus,XYZ(1,1,2).Unitario());
   sol.Imprimir();
   ...
   det = matriz(5,5,500,500,XYZ(1,0,0),XYZ(-1,0,0));
...
//         printf(" (sem colisao)\n");
      ...
      if (hv.Hv()>0.5) {
         ...
         det.Registrar(hv.Pos(),hv.Dir());
      }
//      printf("----------------------------------------------------------------\n");
   }
//   det.Imprimir();
   det.Salvar("imagem.pgm");
